...
import { ArrowLeft, CheckCircle2, Eye, EyeOff, KeyRound, LockKeyhole, Sparkles, UserRound } from "lucide-react";
import "./_group.css";
import "./StudentLogin3D.css";

function LearningOrb() {
  return (
    <div className="login-orb drift absolute left-8 top-10 hidden h-28 w-28 -rotate-6 items-center justify-center rounded-[30px] border border-white/70 bg-[#e9c76a] shadow-[0_14px_0_#c99a42,0_25px_32px_rgba(78,67,41,.18)] md:flex">
          ...
          <div className="grid min-h-[590px] overflow-hidden rounded-[27px] bg-[#fffdf8] lg:grid-cols-[.93fr_1.07fr]">
            <div className="relative flex flex-col justify-between overflow-hidden bg-[#173b50] p-7 text-[#fff8e9] sm:p-10">
              ...
              <div className="relative">
                ...
                <div className="login-companion-wrap" aria-hidden="true">
                  <div className="login-companion-shadow" />
                  <img className="login-companion" src="/__mockup/images/manara-student-login-companion-cutout.png" alt="" />
                  <Sparkles className="login-companion-spark spark-one" aria-hidden="true" />
                  <Sparkles className="login-companion-spark spark-two" aria-hidden="true" />
                </div>
            ...
            <div className="flex items-center p-6 sm:p-12">
              <div className="w-full">
                ...
                <form className="space-y-4" onSubmit={submit}>
                  <label className="block"><span className="mb-2 block text-sm font-extrabold text-[#284658]">اسم المستخدم</span><div className="relative"><UserRound className="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#147d83]" /><input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="اكتب اسم المستخدم" autoComplete="username" className={`world-input w-full rounded-2xl border bg-[#fbfaf5] py-4 pl-4 pr-12 text-right font-bold text-[#183047] placeholder:text-[#9aa9a8] ${showError && !username.trim() ? "border-[#df6d5f]" : "border-[#dbe5df]"}`} /></div>{showError && !username.trim() && <span className="mt-1.5 block text-xs font-bold text-[#bd5148]">اكتب اسم المستخدم للمتابعة</span>}</label>
                  <label className="block"><span className="mb-2 block text-sm font-extrabold text-[#284658]">كلمة المرور</span><div className="relative"><LockKeyhole className="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#147d83]" /><input type={visible ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="اكتب كلمة المرور" autoComplete="current-password" className={`world-input w-full rounded-2xl border bg-[#fbfaf5] py-4 pl-12 pr-12 text-right font-bold text-[#183047] placeholder:text-[#9aa9a8] ${showError && !password ? "border-[#df6d5f]" : "border-[#dbe5df]"}`} /><button type="button" onClick={() => setVisible((v) => !v)} aria-label={visible ? "إخفاء كلمة المرور" : "إظهار كلمة المرور"} className="absolute left-3 top-1/2 -translate-y-1/2 rounded-lg p-2 text-[#6c8285] hover:bg-[#eaf3ef]">{visible ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}</button></div>{showError && !password && <span className="mt-1.5 block text-xs font-bold text-[#bd5148]">اكتب كلمة المرور للمتابعة</span>}</label>
                  {submitted && username.trim() && password && <div className="login-success mt-4 rounded-2xl border border-[#9bd6bd] bg-[#eaf8ef] px-4 py-3 text-center text-sm font-extrabold text-[#28714c]"><CheckCircle2 className="mr-1 inline-block h-4 w-4 align-text-bottom" />أحسنت! تم التحقق، لنبدأ رحلة التعلّم.</div>}
                  <button type="submit" disabled={isSubmitting} className="primary-action mt-7 flex w-full items-center justify-center gap-3 rounded-2xl bg-[#147d83] py-4 text-base font-black text-white shadow-[0_6px_0_#0b5b68] disabled:cursor-wait disabled:opacity-80"><span>{isSubmitting ? "نجهّز رحلتك..." : "ابدأ رحلة التعلم"}</span><ArrowLeft className="h-5 w-5" /></button>
                </form>
