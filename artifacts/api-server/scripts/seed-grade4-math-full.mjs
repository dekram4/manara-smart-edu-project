...
async function main() {
  ...
  // هذا السكربت صار يمحو أكثر مما يبذر.
  //
  // كُتب يوم كانت الرياضيات وحدها في النظام، فحذفُ «كلّ دروس المعلّم
  // والقالب» كان يعني حذف الرياضيات. واليوم معها العلوم والإنجليزية،
  // فالسطر نفسه يمحوهما، وكتابةُ الشجرة إحلالاً تُسقط فرعيهما.
  //
  // ولم يُحذف الملف لأن المحو الكامل يُطلب أحياناً عن قصد. فبقي، ومعه
  // بابٌ لا يُفتح سهواً.
  if (!args.includes("--i-know-this-deletes-other-subjects")) {
    console.error(
      "❌ هذا السكربت يحذف كلّ دروس المعلّم والقالب — بما فيها العلوم واللغة الإنجليزية.\n" +
        "   لتحديث الرياضيات وحدها استعمل:\n" +
        "     node scripts/update-grade4-math-content.mjs --teacher <المعرّف>\n" +
        "   وإن كان المحو الكامل مقصوداً فأضف --i-know-this-deletes-other-subjects.",
    );
    process.exit(1);
  }
