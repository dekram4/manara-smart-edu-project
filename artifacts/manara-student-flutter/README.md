...
`API_BASE_URL` مطلوب عند استخدام مقاطع MP4 المرفوعة من لوحة منارة، لأنه يحدد
عنوان خادم الوسائط لمسار `/api/media/videos/...`. ويُستخدم أيضًا لتفعيل إجابات
المعلم الافتراضي عبر مسار `/api/gemini/answer`. إذا لم يتم تمريره، تعمل الدردشة
بوضع تشجيعي محلي وتحفظ المحادثة في جدول `interactions` عند توفر اتصال Supabase،
لكن لا يمكن للتطبيق تحديد عنوان خادم مقاطع MP4 المحلية.
