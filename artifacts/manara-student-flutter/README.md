...
### تشغيل Flutter Web بمنفذ ثابت

حتى لا يتغير نطاق D‑ID بعد إعادة تشغيل المشروع، استخدم سكربت التشغيل من مجلد
التطبيق:
...
يستخدم السكربت المنفذ `50607`، وهو المنفذ المسجل في D‑ID حاليًا. إذا كان المنفذ
مشغولًا، أوقف جلسة Flutter القديمة بدل استخدام منفذ آخر. ويمكن تغيير المنفذ
عند الحاجة بهذه الطريقة:

```bash
MANARA_FLUTTER_WEB_PORT=50607 bash run_web.sh
```
