...
**How to apply:** Keep new UI, navigation, quiz, and reward events inside the shared audio engine and gate each event to a single playback call. Use a quiet hover cue, unlock hover playback after a pointer/touch/keyboard gesture, and avoid spoken narration in quiz flows unless explicitly requested. Keep the local Arabic student welcome as the only post-login welcome sound.